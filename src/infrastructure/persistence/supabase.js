...
import { collectTrackedSymbols } from '../../domain/pricing/trackedSymbols.js'
...
export async function savePortfolio(userId, assets, settings) {
  ...
  syncTrackedSymbols(assets).catch(e =>
    console.warn('[tracked_symbols] sync failed:', e.message),
  )
  return true
}

/** Upsert symbol registry rows for the price-cache GitHub Action. */
export async function syncTrackedSymbols(assets) {
  if (!supabase) return;

  const rows = collectTrackedSymbols(assets).map(r => ({
    ...r,
    updated_at: new Date().toISOString(),
  }));
...
export async function getPortfolioSnapshots(userId, days = null) {
  ...
  let q = supabase
    ...
    .select('snapshot_date, net_worth_thb, total_invest_thb, total_spec_thb, o_invest_thb, h_invest_thb, l_invest_thb, asset_breakdown')
    .eq('user_id', userId)
...
/** Latest snapshot row for sync health display. */
export async function getLatestSnapshot(userId) {
  if (!supabase || !userId) return null;
  const { data, error } = await supabase
    .from('portfolio_snapshots')
    .select('snapshot_date, snapshot_at')
    .eq('user_id', userId)
    .order('snapshot_date', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error) { console.warn('[Snapshots] latest error:', error.message); return null; }
  return data;
}
